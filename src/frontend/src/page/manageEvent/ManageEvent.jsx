...
export default function ManageEvent() {
    ...
    const handleAddEvent = async (values) => {
        try {
            ...
            const response = await api.post('Events/create', {
                ...
                startTime: values.startTime.toISOString(),
                endTime: values.endTime.toISOString(),
                statusEvent: false, // Đảm bảo statusEvent mặc định là false khi tạo sự kiện
            });
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                // Hiển thị thông báo thành công khi tạo sự kiện
                notification.success({
                    message: 'Event Created',
                    description: 'The event has been successfully created.',
                    placement: 'topRight',
    ...
    const handleAddProductsToEvent = async (eventId) => {
        console.log('Adding products to event:', eventId);
        try {
            // Chuyển eventId thành chuỗi khi gửi
            // const eventIdStr = String(eventId);
            // console.log("Event ID (String):", eventIdStr);
    ...
    const showProductModal = (eventId) => {
        console.log('Showing product modal for event:', eventId);
    ...
    const handleExpandRow = async (eventId) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                const productDetails = Array.isArray(eventData.eventDetails) ? eventData.eventDetails : []; // Đảm bảo eventDetails là mảng, nếu không rỗng sẽ là mảng []
                ...
                console.log(
                    'Updated event:',
                    updatedEvents.find((e) => e.eventId === eventId)
                );
            }
        } catch (error) {
                    ...
                    >
                        ...
                        <Table
                            ...
                            columns={[
                                {
                                    title: 'Select',
                                    render: (_, record) => <Checkbox onChange={() => handleProductSelection(record)} />,
                                },
                                { title: 'Product ID', dataIndex: 'productId', key: 'productId' },
                                { title: 'Product Name', dataIndex: 'productName', key: 'productName' },
                        ...
                        <Button
                            type="primary"
                            onClick={handleAddProductsToEvent.bind(this, selectedEventId)} // Chắc chắn gọi đúng
                            style={{
                                marginTop: '20px',
                                width: '40%',
                                display: 'flex',
